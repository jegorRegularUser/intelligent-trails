window.MapRouteBuilder = {
  ...
  async buildRoute(
    ...
    const waypoints = this.buildWaypointsList(
      startPoint,
      selectedPlaces,
      returnToStart,
      activities
  ...
  async showRouteInfoPanel(waypoints) {
    ...
    setTimeout(() => {
      const segmentData =
        window.MapSmartWalkInstance?.getSegmentData() ||
  ...
  updatePanelWithSegmentData(waypoints) {
    ...
    if (statsDiv) {
      const totalDistance = this.segmentData.reduce(
        (sum, seg) => sum + (seg.distance || 0),
        0
      );
      const totalTime = this.segmentData.reduce(
      ...
      statsDiv.innerHTML = `
                ...
                <div class="route-stat">
                    <span class="stat-icon">📏</span>
                    <span class="stat-value">${(totalDistance / 1000).toFixed(
                      1
                    )}</span>
  ...
  async saveRoute() {
    ...
    const state = window.StateManager?.getState();
    console.log("[MapRouteBuilder] Current state:", state);

    if (!state || !state.routeData) {
      console.error("[MapRouteBuilder] No routeData in state:", state);
      alert("Нет данных маршрута для сохранения");
      return;
    }

    const routeData = state.routeData;
    console.log("[MapRouteBuilder] Route data to save:", routeData);
    ...
    const saveData = {
      start_point: {
        name: (routeData.places && routeData.places[0]?.name) || "Старт",
        coords:
          routeData.start_point ||
          (routeData.places && routeData.places[0]?.coordinates),
      },
      categories: categories,
      time_limit_minutes: totalMinutes || 60,
      return_to_start: routeData.return_to_start || false,
      mode:
        (routeData.places && routeData.places[1]?.transport_mode) ||
        "pedestrian",
      places: routeData.places || [],
    };

    console.log("[MapRouteBuilder] Prepared save data:", saveData);
    console.log("[MapRouteBuilder] Sending to API...");
    ...
    try {
      ...
      console.log("[MapRouteBuilder] Response status:", response.status);
    ...
    waypoints.forEach((point, index) => {
      ...
      const distanceInfo =
        segmentInfo && segmentInfo.distance
          ? `<div class="stage-distance">📏 ${this.formatDistance(
              segmentInfo.distance
            )}</div>`
          : "";
      const timeInfo =
        segmentInfo && segmentInfo.duration
          ? `<div class="stage-time">⏱️ ${this.formatDuration(
      ...
      stagesHTML += `
                <div class="route-stage" onclick="window.MapRouteBuilder.zoomToStage(${index})">
                    ...
                    <div class="stage-content">
                        ...
                        ${
                          point.address
                            ? `<div class="stage-address">📮 ${point.address}</div>`
                            : ""
                        }
                        ${distanceInfo}
                        ${timeInfo}
                        ${
                          point.transport_mode
                            ? `<div class="stage-transport">${transportIcon} ${this.getTransportName(
                                point.transport_mode
                              )}</div>`
                            : ""
                        }
