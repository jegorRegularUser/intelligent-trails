class MapSmartWalk {
  ...
  async visualizeRoute(routeData) {
    ...
    console.log(
      `[MapSmartWalk] Building route through ${places.length} places`
    );

    // Строим каждый сегмент последовательно
  ...
  async drawSegment(fromPlace, toPlace, segmentIndex) {
    ...
    console.log(
      `[MapSmartWalk] Segment ${segmentIndex + 1}: ${fromPlace.name} -> ${
        toPlace.name
      } (${mode})`
    );
    ...
    try {
      ...
      const multiRoute = new ymaps.multiRouter.MultiRoute(
        ...
        },
        {
          ...
          routeActiveStrokeColor: "transparent",
          routeStrokeWidth: 0,
        }
      );
      ...
      if (!activeRoute) {
        console.warn(
          `[MapSmartWalk] No active route for segment ${segmentIndex}`
        );
      ...
      console.log(
        `  ✓ Distance: ${(distance / 1000).toFixed(2)} km, Time: ${(
          duration / 60
        ).toFixed(0)} min`
      );

      // Извлекаем координаты из paths
      const coordinates = [];
      ...
      if (coordinates.length === 0) {
        console.warn(
          `[MapSmartWalk] No coordinates found for segment ${segmentIndex}`
        );
        this.drawFallbackLine(fromCoords, toCoords, mode, segmentIndex);
        return;
      }
      ...
      const polyline = new ymaps.Polyline(
        coordinates,
        {
          balloonContent: this.createBalloonContent(
            fromPlace,
            toPlace,
            distance,
            duration,
      ...
      console.log(`  ✓ Segment ${segmentIndex} drawn successfully`);
    } catch (error) {
      console.error(
        `[MapSmartWalk] Error building segment ${segmentIndex}:`,
        error
      );
      this.drawFallbackLine(fromCoords, toCoords, mode, segmentIndex);
    }
            ...
            <div style="padding: 10px; min-width: 250px;">
                ...
                <div style="display: flex; flex-direction: column; gap: 8px; font-size: 14px;">
                    ...
                    <div style="display: flex; align-items: center; gap: 8px;">
                        ...
                        <span style="color: #555;">${this.formatDistance(
                          distance
                        )}</span>
                    </div>
                    <div style="display: flex; align-items: center; gap: 8px;">
                        <span style="font-size: 18px;">⏱️</span>
                        <span style="color: #555;">${this.formatDuration(
                          duration
                        )}</span>
                    </div>
                </div>
  ...
  convertModeToYandex(mode) {
    // Преобразование режима в формат Yandex API
    const mapping = {
      pedestrian: "walking",
      walking: "walking",
      driving: "driving",
      ...
      masstransit: "transit",
      transit: "transit",
      bicycle: "driving", // Yandex не поддерживает велосипедный режим
    };
